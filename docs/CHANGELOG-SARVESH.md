...
**Review round 1 (same day, independent reviewer).** The words were fixed in three files but four other emitters reached the same screen. Closed: the reading's anchor chips ("DMARC: missing", "SSL grade: B", "3 corpus holdings", "Company research · perplexity + gemini", "Oracle hosting") now read "how mail is set up", "how the connection is set up", "3 notes we already held", "the live web, two sources", and the probe-derived ones never become chips; an extraction failure no longer prints the raw error; a skipped engine says "no answer this time", never "quota exhausted" or "engine error (503)"; the work panel counts "Answers from the web" instead of naming engines; the vendor's letter grade is not restated; a non-public address is narrated in words; a source-scan test now sweeps every handler and service for machinery words and raw errors in screen lines. In the play copy, the outside look folds by default and the sidebar, opener and chips all skip probe facts.

**Review round 2 (same day, second independent reviewer).** Three of my own mistakes from round 1, caught: the `.gitignore` rewrite for the symlink had dropped the other 31 lines (`api/.env` and the meter data were un-ignored for one commit; restored in full); the new source-scan test never matched `appendLog(id, {…})` calls, so it guarded nothing in the analyze handler (rewritten to capture each emitter call to its own closing parenthesis, multi-line included, and proved on a fixture; a third review found the first rewrite started one character late and never closed, fixed the same day); and the plain corpus line had blanked the play copy's Holdings panel, which parsed the old words (the hit now rides beside the line as data, and the panel reads that). Also: the play copy ranks acts so the outside look never leads; a failure before any act started shows under the read, never under the outside-look heading; the facts handed to the reading model use plain words too and the outside-look facts sit after every business fact under a "background, never the opener" header, with the identity override after everything, so the rule is in the evidence order, not only in the prose; open ports are stated as reachable, never as what "should" be; the meter row keeps the page's status code.

**Review round 4 (15 Sept, fourth independent reviewer, scoped to the round-3 fixes).** One hole and one unproven claim. The hole: the source-scan test counted every parenthesis and brace in the file, including ones inside quoted text, so a screen line whose words contained an unmatched "(" made the scanner drop that whole call, and a machinery word or raw error sitting beside it went unchecked. The scanner now ignores punctuation inside strings, template literals and comments, and a fixture with unbalanced punctuation in the text pins it. The claim: "the connection line is a fact, not a verdict" had been proven for an A grade only. The two other branches are now pinned by test, and the fallback wording changed from "encryption below current best practice" (a judgement) to "encryption on, current versions only, some settings behind the current standard" (what is on). Everything else in round 3 held: the evidence order is in the array build, not the comments; an error while the outside look is open goes to the read; no "should" reaches the screen.
...
A second seam of the same class sat one file from a fix that already existed: `corpus-retrieve.js` keeps three states apart (hits / `[]` = looked, found nothing / `null` = could not look), and both chat handlers wrote the receipt as `hits: corpusHits ?? []`, so "the evidence store was unreachable" became "no sources retrieved" on the founder's screen.

**Fix.** An absence now travels as its own state on every hop and is never voiced as a finding. The compliance placeholder is `{ label: 'SOC 2: not seen on the pages read', confidence: 'not_observed', state: 'not_observed' }` (same `inference_id`, so `early-signal.js` keys still match). The normaliser derives `'unknown'` from it, never `'none'`. Gap definitions gain an optional `absenceCondition(ctx)`; a gap that fires from one carries `state: 'not_observed'`. The persona prompt filters those out of "Gaps identified" and lists them under a new heading, *Not seen on the pages read (absences, NOT findings)*, with the register spelled out: say "we did not see", never "your gap" or "you lack". The frontend grade word for such a signal is *not seen*, not *probable*. For the second seam, `toReceiptHits(null)` returns `null`, both handlers pass the retrieval result through untouched, and the "Our working" panel reads *could not look (evidence store unreachable this turn)* instead of *no sources retrieved*. Twelve new API tests (`absence-not-observed.test.js`) pin each hop; four frontend tests pin the two renderers; two existing receipt tests that asserted the old collapse were rewritten to assert the new contract.

**Round 2, same day.** An independent review of that first cut (a second model, not the one that wrote it) found the absence still reaching the founder by four other doors, and a fifth waiting. The penetration-testing gap fires from the same "compliance not seen" and was untagged, so it was voiced as a finding while SOC 2 was not. The "How we read this" panel listed the absence as a gap that "carried weight", with its reasoning, and subtracted 20 for it. The vendor selector matched a *start here* vendor to it. The "founder is currently looking at this gap" prompt block had no guard (unreachable today, wired for the day a gap card is clickable). And the very first chat message printed the raw enum, "(not_observed)". All closed the same way: the pen-test gap gets an absence condition (a founder's "no" is testimony and stays observed); an absence carries `score_impact: 0`, so neither the trust score, the early estimate, nor the ledger panel subtracts anything for what we did not see; `selectVendors` matches observed gaps only (the VERITAS recompute path tags its attested gaps observed); the active-gap block refuses an absence; the opener lists an absence by its label alone. Seven more tests, one per door. API 577/577, frontend 536/536.

**Why it matters.** Law 5 of the reading: *observed, not confirmed*. The product may say what it saw and what it did not see; it may never turn "not seen" into "you lack". The fix is at the mechanism, not in the prompt wording, because a rule the model can decline is not a control. This is fix 1 of 4 from the walk; it closes only when the same beat is re-walked by a reviewer who did not build it and Sophia's answer holds no claim the record does not.
...
## 2026-09-14 — HX finding 8 + R8: absence is the default, and nothing marks the founder

- **Default-deny gap state.** A gap is `observed` only when its definition's `observedBy(ctx)` says a probe saw it or the founder said it (`api/src/config/gaps.js`, `gap-mapper.js`, `recompute.js`). Everything else is `not_observed`: zero weight, outside "Gaps identified", buys no vendor, no consequence copy (why = the peer sentence or null, risk null). Nine ids are allow-listed as always-observed (seven perimeter probes, two program eligibilities) in `api/tests/unit/absence-default-deny.test.js`; adding one is a reviewed act.
- **R8 language.** No "you don't / you haven't / you lack / you have no / you can't / your gap" anywhere a founder reads. The absence sentence is "Looking at your site and around the web, we couldn't spot one. Do you have it?" (`peer-reference.js`); the persona absence block says the same and never hands the advisor a "<x> gap" noun. Runtime why/risk tables rewritten; Hive demo and UI labels rewritten. Pinned by `api/tests/unit/no-negative-language.test.js` (the copy that ships, not the definitions) and `frontend/tests/unit/no-negative-language.source.test.js` (every UI file, comments stripped).
- **Honest consequence:** SOC 2 can only become observed from founder testimony, and no path lets the founder say it yet. Until the R9 interview ladder lands, SOC 2 is always an absence, said as one.
...
## 2026-09-14 — Inbound trust check: the mailbox door (check@ethikslabs.com)
