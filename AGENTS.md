...
```
ANTHROPIC_API_KEY=...      # Codex Haiku for signal extraction
...
POST /api/session/start
  → signal-extractor.js  (Firecrawl scrape + Codex extraction)
...
| `api/src/services/signal-extractor.js` | Firecrawl → Codex → raw signals (product_type, data_sensitivity, compliance_status, etc.) |
