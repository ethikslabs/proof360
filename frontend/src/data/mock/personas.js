...
const INTENT_RESPONSES = {
  ...
  stripe: {
    leonardo: "Stripe isn't just payments — it's the financial infrastructure layer that investors and acquirers check. Three products matter at different stages: Stripe Atlas if incorporation isn't clean yet (Delaware C-corp, done in a day), Stripe Capital for non-dilutive financing once you have 6+ months of revenue history, and Stripe Revenue Recognition if you're on any kind of subscription model — that's what makes your MRR investor-grade rather than founder-claimed.\n\nSophia — from the investor lens, what does Stripe presence actually signal?",
