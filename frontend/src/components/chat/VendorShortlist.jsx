...
export function VendorShortlist({ vendors, shortlistedIds, onShortlist, onDefer }) {
  const byTiming = {
        ...
        byTiming[timing].length > 0 && (
          <div key={timing} style={{ marginBottom: 20 }}>
            ...
            {byTiming[timing].map(v => {
              const isShortlisted = shortlistedIds?.some(s => (s.id || s) === v.id);
              const shortlistEntry = shortlistedIds?.find(s => (s.id || s) === v.id);
              return (
                <div key={v.id} style={{
                  marginBottom: 10,
                  padding: '12px 14px',
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    {isShortlisted ? (
                      <>
                        ...
                        {shortlistEntry?.provenance?.added_at && (
                      ...
                      <>
                        <button
                          onClick={() => onShortlist({ ...v, provenance: { added_at: new Date().toISOString() } })}
                          style={{ fontSize: 11, padding: '4px 10px', borderRadius: 6, border: `1px solid ${TIMING_STYLE[timing].color}`, background: 'transparent', color: TIMING_STYLE[timing].color, cursor: 'pointer' }}
                        >
                          + Add to shortlist
                        </button>
                        <button
                          onClick={() => onDefer?.(v.id)}
                          style={{ fontSize: 11, padding: '4px 10px', borderRadius: 6, border: '1px solid #1e293b', background: 'transparent', color: '#475569', cursor: 'pointer' }}
                        >
                          Defer
                        </button>
                      </>
