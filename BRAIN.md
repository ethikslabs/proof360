...
**Written:** Sun 13 Sept 2026, Claude Code (Edison), at proof360 `2bf544e`; rulings R8, R9, R10 folded in Mon 14 Sept 2026; law 5 class fix the same day. **Reader:** Sarvesh.
...
|---|---|---|---|---|
| 1 | **Read the business before the wiring.** Never open on a technical fact. | reading · **interview order** | **no test** | **Ruled twice, not built.** 13 Sept: "start with the business, always." **14 Sept (R9): the interview is an investor's first three questions, then on with it** (type of company, industry, stage, size), never an interrogation, nothing technical ever asked. `ASK_ORDER` still opens on `infrastructure.cloud_provider` (`Interview.jsx:35`). Build owed; the pinning test is `interview-flow` asserting the first ask is a business field and the fourth ask does not exist. |
...
| 5 | **Absence of evidence is never evidence of absence.** Found / honest zero / could not look are three states, and "not seen" is never "you lack". | reading · **personas** · **gap copy** · **register triggers** · **UI labels** (receipts, opener, ledger) | `api/tests/unit/absence-not-observed.test.js` (every hop) · `absence-default-deny.test.js` (the default: observed only by probe or founder word; nine allow-listed ids; both gap engines) · `no-negative-language.test.js` (the copy that ships) · `frontend/tests/unit/no-negative-language.source.test.js` (every UI file) · `chat-receipts.test.js` · `session-chat-grounding.test.js` · `register-advisory.test.js` · `frontend/tests/unit/absence-not-observed.render.test.jsx` · `OurWorking.test.jsx` | **Class fix built 14 Sept (finding 8 + R8), two review rounds, awaiting the re-walk.** A gap is `observed` only when its `observedBy` says a probe saw it or the founder said it; everything else is an absence: zero weight, outside "Gaps identified", buys no vendor, no consequence copy, spoken as *we* + the method. Honest residual: SOC 2 can only be observed from founder testimony and no path lets the founder say it yet (the R9 ladder), so it is always an absence until then. |
| 5a | **The absence register is the spoken line:** "Companies like yours usually go for X to achieve Z, and use Y to get there. Looking at your site and around the web, we couldn't spot one. Do you have it?" *"This is how I speak in human."* (R6, 13 Sept; closing rung rewritten by R8, 14 Sept: never "we didn't see" as a mark, always *we looked, here is how, we couldn't spot it*) | personas · gap copy · UI labels (opener) | `api/tests/unit/peer-reference.test.js` (every slot derived; null when the cohort was not observed or does not usually go for X; never "need" / "your gap"; stake disclosed) | **Built 13 Sept; not yet heard in a walk.** `api/src/services/peer-reference.js`. In the lxplatform re-walk Sophia kept the shape ("we didn't see … do you have one") but the peer sentence was not spoken and the reading channel never carries it (finding 14). The R8 closing rung is not yet in the code. **Change the sentence by changing the table, never by editing the prompt.** |
...
| 7 | **Infer first, then ask one thing, once.** Not knowing is an answer. | interview order · reading | `api/tests/unit/interview-flow.test.js` ("I don't know" is a first-class answer; an answered claim is never asked again) · `confirm-ask-fatigue.test.js` (stops asking a question nobody is answering) | Holds as mechanism; three answers wired (yes / correct / don't know). **R10 (14 Sept): technical claims (WAF, CDN, DMARC, email provider, headers) are collected, never asked; surfaced only when a chat turn makes them relevant.** The confirm ceremony's candidate pool must exclude probe-kind claims; today it does not (the lxplatform ledger shows six probe facts reading "your word"). Pin: `confirm-ceremony` never selects a claim whose provenance method is a probe. |
| 7a | **The ability to answer is the read.** A short ladder of questions only the real role can answer; an unanswered rung is a *state* on the record, never a verdict, and the judgment stays in John's head. ("Are you an investor? What industries? What stage? What ticket size?") 13 Sept. | interview order · reading · personas | **no test** | Ruled as method; **R9 (14 Sept) gives the founder's ladder its first three rungs: type of company, industry, stage, size, and no fourth.** The closing question of 5a is a later rung, asked only in conversation. Intake for partners and investors runs the same shape. |
...
| 9 | **Never pitch before they have spoken.** A decline is remembered forever. | **register triggers** · shortlist | `api/tests/unit/trigger-evaluator.test.js` (confirmed claims only, D4) — pins the evaluator, not the latch | **The gap-kind latch turns out to be dead; the type-kind latch is live.** Review 14 Sept: `shortlist.js` reads `g.id` while gap-mapper emits `gap_id`, so the 40 gap-kind register entries have never fired. Every pitch in the lxplatform walk (three programs in two turns, finding 10) came from type-kind entries firing on "product type is confirmed as B2B SaaS". Left untouched on purpose: fixing the field name would switch the latch on. Unruled: whether a gap-kind or type-kind trigger needs a founder word. |
