...
**Written:** Sun 13 Sept 2026, Claude Code (Edison), at proof360 `2bf544e`; rulings R8, R9, R10 folded in Mon 14 Sept 2026. **Reader:** Sarvesh.
...
| 5 | **Absence of evidence is never evidence of absence.** Found / honest zero / could not look are three states, and "not seen" is never "you lack". | reading · **personas** · **gap copy** · **register triggers** · **UI labels** (receipts, opener, ledger) | `api/tests/unit/absence-not-observed.test.js` (every hop: inference → normaliser → gap state → prompt block; receipt keeps `null` ≠ `[]`; score, vendor, active-gap, opener doors) · `chat-receipts.test.js` · `session-chat-grounding.test.js` · `register-advisory.test.js` ("the honest zero is an answer, not an apology") · `frontend/tests/unit/absence-not-observed.render.test.jsx` · `OurWorking.test.jsx` — **no test for the words** (see R8) | **Pinned 13 Sept for two gaps only (HX fix 1, commits 5123f4b · 83a431f · 2bf544e); the class is open.** `absenceCondition` is opt-in: only `soc2` and `penetration_testing` carry one, every other gap is born observed and `gap-mapper.js` stamps it confirmed. The 13 Sept re-walk (lxplatform.io) held on the SOC 2 door and failed on the privacy door: "you don't yet have a published Privacy Policy" (finding 8). Class fix owed: default-deny, a gap is `not_observed` unless a probe or the founder spoke to what its claim asks. **R8 (14 Sept): no second-person deficit ever, in any channel; an absence is said as *we* + the method** ("scanning your site and having a look around the web, we couldn't spot a …"), brotherly, the primary-school teacher's register. Pin: a string-level test over rendered copy and prompt blocks. |
...
| 9 | **Never pitch before they have spoken.** A decline is remembered forever. | **register triggers** · shortlist | `api/tests/unit/trigger-evaluator.test.js` (confirmed claims only, D4) — pins the evaluator, not the latch | **A latch, not a gate, now seen in a walk.** 40 of 70 register entries are gap-kind and fire on derived gaps with `claims_cited: []`; type-kind entries fire on "product type is confirmed as B2B SaaS". lxplatform re-walk, 13 Sept: three programs in two turns, each ending "want me to add that as well?" (finding 10). Since 13 Sept an *absence*-derived gap no longer selects a vendor (5). Unruled: whether a gap-kind or type-kind trigger needs a founder word. |
